...
//            CC_d = d_rirj(r[i], r[j]);
            ...
            if (CC_r < RCUT || R_CUT == -1) {
                ...
                F.x += - 6.*Cij/CC_r8*CC_d.x - 8.*Dij/CC_r10*CC_d.x + C_DVCUT[indx_int]/CC_r*CC_d.x + C_DDVCUT[indx_int]*(CC_r - RCUT)/CC_r*CC_d.x;
...
//            SS_d = d_rhoirhoj(rho[i], r[i], rho[j], r[j]);
            ...
            if (SS_r < RCUT || R_CUT == -1) {
                ...
                F.x += Aij/LAMBDA*SS_d.x/SS_r*exp(-SS_r/LAMBDA) + S_DVCUT[indx_int]/SS_r*SS_d.x + S_DDVCUT[indx_int]*(SS_r - RCUT)/SS_r*SS_d.x;
